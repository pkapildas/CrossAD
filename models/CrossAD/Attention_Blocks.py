...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_mask=None):
        out = F.scaled_dot_product_attention(
            q, k, v, attn_mask=attn_mask, dropout_p=0. if not self.training else self.attn_dropout.p
        )
        return out, None
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        k_s = self.W_K(K).view(bs, -1, self.n_heads, self.d_k).permute(0,2,1,3)       # k_s: [bs x n_heads x k_len x d_k]
